...
void InsulatingheatPropertyWidget::initWidget()
{
	...
	// 单位列
	QStringList unitLabels = { " "," ", "kg/m^3", "/K", "MPa"," ","MPa","MPa","W/m K","J/kg K" };
...
void InsulatingheatPropertyWidget::showTableDialog()
{
	...
	QDir dir;
	QString filepath = dir.absoluteFilePath("src/database/防隔热材料.xlsx");
	int m_rowCount = 0;
	...
	QString privateFilePath = dir.absoluteFilePath("src/database/" + info.username + "/防隔热材料.xlsx");
		...
		if (row != 0)
		{
			...
			while (parent) {
				...
				if (gfParent)
				{
					...
					QString text = timeStr + "[信息]>开始导入防隔热材料数据";
