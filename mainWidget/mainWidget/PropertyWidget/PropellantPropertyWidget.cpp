...
void PropellantPropertyWidget::initWidget()
{
	...
	// 单位列
	QStringList unitLabels = { " "," "," ", "kg/m^3", "/K", "GPa"," ","℃","MPa","W/m K","J/kg K"," "," "," "," "," "," "," " ," " ," " ," " ," " ," " };
...
void PropellantPropertyWidget::showTableDialog() 
{
	...
	connect(diaTableWidget, &QTableWidget::cellDoubleClicked, this, [this, dialog, diaTableWidget](int row, int column) {
		if (row != 0)
		{
			...
			info.ignitionTemperature = m_tableWidget->item(7, 2)->text().toDouble();
			info.fireOverpressure = m_tableWidget->item(8, 2)->text().toDouble();
			...
			info.i = m_tableWidget->item(11, 2)->text().toDouble();
			info.a = m_tableWidget->item(12, 2)->text().toDouble();
			info.b = m_tableWidget->item(13, 2)->text().toDouble();
			info.c = m_tableWidget->item(14, 2)->text().toDouble();
			info.d = m_tableWidget->item(15, 2)->text().toDouble();
			info.g_1 = m_tableWidget->item(16, 2)->text().toDouble();
			info.e = m_tableWidget->item(17, 2)->text().toDouble();
			info.g = m_tableWidget->item(18, 2)->text().toDouble();
			info.x = m_tableWidget->item(19, 2)->text().toDouble();
			info.y = m_tableWidget->item(20, 2)->text().toDouble();
			info.z = m_tableWidget->item(21, 2)->text().toDouble();
			info.g_2 = m_tableWidget->item(22, 2)->text().toDouble();
			info.isChecked = true;
